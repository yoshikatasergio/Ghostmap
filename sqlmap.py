...
def main():
    ...
    try:
        ...
        # GhostMap addition: runtime integrity check (Python version,
        # critical modules, optional capabilities). Reports via the same
        # logger sqlmap uses; non-blocking unless a critical module is
        # missing.
        try:
            from lib.core.integrity import run_integrity_check
            from lib.core.log import LOGGER as _gm_logger
            _gm_integrity = run_integrity_check(logger=_gm_logger)
            if _gm_integrity.get("summary") == "error":
                dataToStdout("[x] GhostMap integrity check failed; aborting.\n", forceOutput=True)
                os._exitcode = 1
                raise SystemExit(1)
        except SystemExit:
            raise
        except Exception as _gm_ex:
            # Never let the integrity check itself break sqlmap.
            try:
                from lib.core.log import LOGGER as _gm_logger
                _gm_logger.debug("GhostMap integrity check skipped: %s" % _gm_ex)
            except Exception:
                pass

        # GhostMap addition: --sergio diagnostic collector (redacted).
        # Only activates when --sergio is on the command line.
        try:
            if getattr(conf, "sergio", False):
                from lib.core.sergio import setup_collector, install_excepthook
                from lib.core.log import LOGGER as _gm_logger
                setup_collector(_gm_logger)
                install_excepthook()
                _gm_logger.info("[--sergio] diagnostic collection ENABLED. "
                                "A redacted dump will be written when GhostMap exits. "
                                "The dump excludes target host, URL path, and parameter values.")
        except Exception as _gm_ex:
            try:
                from lib.core.log import LOGGER as _gm_logger
                _gm_logger.debug("GhostMap --sergio init failed: %s" % _gm_ex)
            except Exception:
                pass
        ...
        # GhostMap addition: write the --sergio diagnostic dump if active.
        try:
            from lib.core.sergio import is_active, finalize as _sergio_finalize
            if is_active():
                from lib.core.data import paths as _gm_paths
                _out = _gm_paths.get("SQLMAP_OUTPUT_PATH") or os.path.expanduser("~")
                _path = _sergio_finalize(_out)
                if _path:
                    dataToStdout("[--sergio] diagnostic dump written:\n", forceOutput=True)
                    dataToStdout("           %s\n" % _path, forceOutput=True)
                    dataToStdout("[--sergio] safe to share with the fork maintainer\n", forceOutput=True)
        except Exception:
            pass
