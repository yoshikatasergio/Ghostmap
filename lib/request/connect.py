...
class Connect(object):
    ...
    def getPage(**kwargs):
        ...
        with kb.locks.request:
            ...
            # GhostMap addition: count requests for --sergio diagnostics.
            # This sees every outgoing request (including retries).
            try:
                from lib.core.sergio import get_collector
                _gm_coll = get_collector()
                if _gm_coll is not None:
                    _gm_coll.note_request()
            except Exception:
                pass
        ...
        try:
            ...
            # GhostMap addition: capture success status code for --sergio.
            try:
                from lib.core.sergio import get_collector
                _gm_coll = get_collector()
                if _gm_coll is not None and code is not None:
                    bucket = "responses_%dxx" % (int(code) // 100)
                    if bucket in _gm_coll.stats:
                        _gm_coll.stats[bucket] += 1
            except Exception:
                pass
            ...
            # GhostMap addition: record status code for --sergio.
            try:
                from lib.core.sergio import get_collector
                _gm_coll = get_collector()
                if _gm_coll is not None:
                    # Re-count this response without double-counting the request.
                    _gm_coll.stats["requests_sent"] = max(0, _gm_coll.stats["requests_sent"])
                    bucket = "responses_%dxx" % (code // 100)
                    if bucket in _gm_coll.stats:
                        _gm_coll.stats[bucket] += 1
            except Exception:
                pass
            ...
            # GhostMap addition: count timeouts/connection errors for --sergio.
            try:
                from lib.core.sergio import get_collector
                _gm_coll = get_collector()
                if _gm_coll is not None:
                    if "timeout" in tbMsg.lower():
                        _gm_coll.stats["timeouts"] += 1
                    else:
                        _gm_coll.stats["connection_errors"] += 1
            except Exception:
                pass
