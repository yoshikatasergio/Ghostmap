...
def _setRequestParams():
    ...
    # GhostMap addition: detect the classic Windows-CMD pitfall where
    # the user did `python sqlmap.py -u https://x/?a=1&b=2&c=3` (no
    # quotes), and CMD interpreted `&` as a command separator, so the
    # process actually received only `-u https://x/?a=1`.
    #
    # Symptom from the operator's POV: GhostMap "doesn't find" the other
    # parameters they thought they had passed. Symptom from CMD's POV:
    # warnings like:
    #   '"b"' no se reconoce como un comando interno o externo
    # printed AFTER GhostMap exits.
    #
    # We can't see CMD's stderr, but we can detect the most common form:
    # the URL has exactly one query parameter AND ends in something that
    # looks like it was the start of another parameter (e.g. ends with a
    # value where the next `&otherparam=val` got chopped off).
    try:
        _gm_url = conf.url or ""
        if _gm_url and "?" in _gm_url and "&" not in _gm_url:
            # Single-parameter URL on Windows: warn loud.
            import platform as _gm_plat
            if _gm_plat.system() == "Windows":
                _q = _gm_url.split("?", 1)[1]
                # If there's exactly one '=' and no '&', this might be the
                # full intended URL OR it might be a CMD-clobbered URL.
                # Either way, show a clear hint -- it's free.
                if "=" in _q:
                    logger.warning(
                        "Windows CMD note: GhostMap received a URL with only "
                        "one query parameter ('%s'). If your real URL had "
                        "additional '&parameter=value' segments, CMD ate them "
                        "because '&' is a command separator. Wrap the URL in "
                        "double quotes:  -u \"https://...?a=1&b=2&c=3\"" %
                        _q
                    )
    except Exception:
        pass
    ...
    if conf.data is not None:
        ...
        if re.search(JSON_RECOGNITION_REGEX, conf.data):
            # GhostMap addition: enumerate injection points and display
            # them to the operator BEFORE any HTTP request is sent.
            # This is purely informative; the marking logic below is
            # unchanged from upstream sqlmap.
            try:
                from lib.core.jsonparser import enumerate_injection_points, format_summary
                _ghostmap_points = enumerate_injection_points(conf.data)
                if _ghostmap_points:
                    logger.info("JSON body contains %d candidate injection point%s:" % (
                        len(_ghostmap_points),
                        "" if len(_ghostmap_points) == 1 else "s",
                    ))
                    for _line in format_summary(_ghostmap_points).splitlines():
                        dataToStdout(_line + "\n")
            except Exception as _ex:
                logger.debug("GhostMap JSON enumeration skipped: %s" % _ex)
