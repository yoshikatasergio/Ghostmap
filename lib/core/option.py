...
def _cleanupOptions():
    ...
    # GhostMap addition: --engagement redirects output to a per-client
    # subfolder. We must re-call setPaths so the new path takes effect,
    # since cmdLineParser runs AFTER the initial setPaths in main().
    if conf.get("engagement"):
        setPaths(paths.SQLMAP_ROOT_PATH)
