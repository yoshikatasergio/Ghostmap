...
# GhostMap addition: rebrand "sqlmap" -> "ghostmap" in user-visible log
# output without touching the (many) call sites that compose those
# messages. The original strings remain unchanged in the source for
# upstream-merge compatibility.
class _GhostMapFormatter(logging.Formatter):
    _BRAND_PATTERNS = (
        (re.compile(r"\bsqlmap is going\b"),         "ghostmap is going"),
        (re.compile(r"\bsqlmap will\b"),             "ghostmap will"),
        (re.compile(r"\bsqlmap has\b"),              "ghostmap has"),
        (re.compile(r"\bsqlmap was\b"),              "ghostmap was"),
        (re.compile(r"\bsqlmap could\b"),            "ghostmap could"),
        (re.compile(r"\bsqlmap detected\b"),         "ghostmap detected"),
        (re.compile(r"\bsqlmap got\b"),              "ghostmap got"),
        (re.compile(r"\bsqlmap identified\b"),       "ghostmap identified"),
        (re.compile(r"\bsqlmap resumed\b"),          "ghostmap resumed"),
        (re.compile(r"\bsqlmap needs\b"),            "ghostmap needs"),
        (re.compile(r"\bsqlmap requires\b"),         "ghostmap requires"),
        (re.compile(r"\bsqlmap retrieved\b"),        "ghostmap retrieved"),
        (re.compile(r"\bsqlmap found\b"),            "ghostmap found"),
        (re.compile(r"\bsqlmap recognized\b"),       "ghostmap recognized"),
        (re.compile(r"\bsqlmap '"),                  "ghostmap '"),
        # Don't touch URLs or identifiers (lib.core.foo, sqlmap.org,
        # github.com/sqlmapproject, etc.) -- the patterns above only
        # match the standalone word followed by a verb/space.
    )

    def format(self, record):
        msg = super(_GhostMapFormatter, self).format(record)
        for pat, repl in self._BRAND_PATTERNS:
            msg = pat.sub(repl, msg)
        return msg

FORMATTER = _GhostMapFormatter("\r[%(asctime)s] [%(levelname)s] %(message)s", "%H:%M:%S")
