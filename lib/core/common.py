...
def setPaths(rootPath):
    ...
    if IS_WIN:
        # GhostMap rebrand: workspace folder is "ghostmap" so a parallel
        # sqlmap install on the same machine doesn't share/clobber output,
        # history, or session files. Reference for env var paths:
        # https://pureinfotech.com/list-environment-variables-windows-10/
        if os.getenv("LOCALAPPDATA"):
            paths.SQLMAP_HOME_PATH = os.path.expandvars("%LOCALAPPDATA%\\ghostmap")
        elif os.getenv("USERPROFILE"):
            paths.SQLMAP_HOME_PATH = os.path.expandvars("%USERPROFILE%\\Local Settings\\ghostmap")
        else:
            paths.SQLMAP_HOME_PATH = os.path.join(os.path.expandvars(os.path.expanduser("~")), "ghostmap")
    else:
        # GhostMap rebrand: ~/.ghostmap (legacy) or ~/.local/share/ghostmap (XDG).
        paths.SQLMAP_HOME_PATH = os.path.join(os.path.expandvars(os.path.expanduser("~")), ".ghostmap")

        if not os.path.isdir(paths.SQLMAP_HOME_PATH):
            if "XDG_DATA_HOME" in os.environ:
                paths.SQLMAP_HOME_PATH = os.path.join(os.environ["XDG_DATA_HOME"], "ghostmap")
            else:
                paths.SQLMAP_HOME_PATH = os.path.join(os.path.expandvars(os.path.expanduser("~")), ".local", "share", "ghostmap")

    # GhostMap addition: --engagement appends a subfolder for the named
    # engagement so multi-client output doesn't mix.
    _gm_engagement = ""
    try:
        from lib.core.data import conf as _gm_conf
        if _gm_conf.get("engagement"):
            _gm_engagement = re.sub(r"[^A-Za-z0-9._-]", "_", _gm_conf.engagement)[:64]
    except Exception:
        _gm_engagement = ""

    if _gm_engagement:
        paths.SQLMAP_OUTPUT_PATH = getUnicode(
            os.path.join(paths.SQLMAP_HOME_PATH, "output", _gm_engagement),
...
def safeFilepathEncode(filepath):
    ...
    >>> 'ghostmap' in safeFilepathEncode(paths.SQLMAP_HOME_PATH)
    True
    """
