...
def checkSqlInjection(place, parameter, value):
    ...
    while tests:
        ...
        try:
            ...
            # GhostMap addition: register every technique that gets tested
            # (whether or not it confirms) for --sergio diagnostics.
            try:
                from lib.core.sergio import get_collector
                _gm_coll = get_collector()
                if _gm_coll is not None:
                    _gm_tech_name = PAYLOAD.SQLINJECTION.get(stype, "stype-%s" % stype)
                    _gm_coll.note_technique(_gm_tech_name, confirmed=False)
            except Exception:
                pass
            ...
            for boundary in boundaries:
                ...
                for where in test.where:
                    ...
                    if injectable is True:
                        ...
                        # GhostMap addition: register the confirmed technique
                        # in the --sergio diagnostic stats.
                        try:
                            from lib.core.sergio import get_collector
                            _gm_coll = get_collector()
                            if _gm_coll is not None:
                                _gm_tech_name = PAYLOAD.SQLINJECTION.get(stype, "stype-%s" % stype)
                                _gm_coll.note_technique(_gm_tech_name, confirmed=True)
                        except Exception:
                            pass
...
def checkWaf():
    ...
    if retVal:
        ...
        # GhostMap addition: try to identify the specific WAF vendor
        # using updated 2026 fingerprints. Detection only -- no bypass.
        try:
            from lib.core.wafdetect import detect as gm_waf_detect
            from lib.core.wafdetect import format_detection_report as gm_waf_report

            headers_text = ""
            cookies_text = ""
            body_text = kb.originalPage or ""

            try:
                if kb.headers:
                    headers_text = "\n".join("%s: %s" % (k, v) for k, v in kb.headers.items())
                    cookies_text = kb.headers.get("Set-Cookie", "") or kb.headers.get("set-cookie", "") or ""
            except Exception:
                pass

            gm_detections = gm_waf_detect(headers_text, cookies_text, body_text)
            if gm_detections:
                report = gm_waf_report(gm_detections, color=True)
                dataToStdout(report)
                kb.identifiedWafs = set(d["name"] for d in gm_detections)
        except Exception:
            # Detection failure must NEVER break the scan flow.
            pass
