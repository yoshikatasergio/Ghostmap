        ...
        if not all((validPayload, vector)) and not all((conf.uChar, conf.dbms, kb.unionTemplate)):
            if Backend.getIdentifiedDbms() and kb.orderByColumns and kb.orderByColumns < FUZZ_UNION_MAX_COLUMNS:
                if kb.fuzzUnionTest is None:
                    # GhostMap improvement: be explicit about what the
                    # fuzzy test does and what the cost is, so the
                    # operator can answer informedly. Skip silently when
                    # running --batch (the default of N is correct, no
                    # need for the noisy line).
                    if conf.get("batch"):
                        kb.fuzzUnionTest = False
                    else:
                        msg = "couldn't confirm UNION-based injection with "
                        msg += "%d column(s). Try fuzzy column-type detection? " % kb.orderByColumns
                        msg += "(slower; fires extra requests testing every "
                        msg += "DBMS-specific data type at each column slot) [y/N] "
