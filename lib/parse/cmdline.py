...
def cmdLineParser(argv=None):
    ...
    try:
        ...
        # GhostMap addition: --sergio collects a redacted diagnostic dump
        # so the fork maintainer can debug bugs without seeing the target's
        # URL or sensitive parameter values.
        miscellaneous.add_argument("--sergio", dest="sergio", action="store_true",
            help="[GhostMap] Collect redacted diagnostic dump for fork debugging")

        # GhostMap addition: --engagement separates output by client/project
        # so a multi-engagement workflow doesn't mix data.
        miscellaneous.add_argument("--engagement", dest="engagement",
            help="[GhostMap] Tag scan with engagement name (separates output by client)")
